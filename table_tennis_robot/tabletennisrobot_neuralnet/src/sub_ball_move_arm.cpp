...
void readTrajectoryFromData(){
  std::ifstream fin;
  fin.open("/home/denn1slu/Dennis/pingpong_trajectory/30fps_test_gazebo.csv", std::ios::in);
  //fin.open("/home/denn1slu/ros_workspace/vscode_ws/src/table_tennis_robot/pingpong_description/models/pingpong.sdf", std::ios::in);
  //std::cout << "###############"<<std::endl;
  //std::cout << fin.rdbuf();
  //std::cout << "###############"<<std::endl;
  std::string line;
...
int main(int argc, char **argv)
{
  ...
  ros::ServiceClient setModelState_client = n.serviceClient<gazebo_msgs::SetModelState>("gazebo/set_model_state");
  ...
  setModetState_srv.request.model_state.twist.linear.x = (matrix.at(3) - matrix.at(0))*30;
  setModetState_srv.request.model_state.twist.linear.y = (matrix.at(4) - matrix.at(1))*30;
  setModetState_srv.request.model_state.twist.linear.z = (matrix.at(5) - matrix.at(2))*30;
