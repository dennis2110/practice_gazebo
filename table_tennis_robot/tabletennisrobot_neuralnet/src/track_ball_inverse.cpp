...
int main(int argc, char **argv)
{
  ...
  while (ros::ok())
  {
    ...
    if(is_ball_left)
      ...
      inv_y1 = ball_y - L3;
    inv_z1 = ball_z;
    inv_y0 = 0.4735 - unified_cmd.data.at(0);
    inv_z0 = 0.94; //0.937
    ...
    //is J1 need change
    if((ball_y >= 0.765 && is_ball_left) || (ball_y <= -0.765 && is_ball_right))
      changeside = true;
    else
      changeside = false;
    ...
    if((ball_y > -0.765 && ball_y < 0.765) && (changepos || changeside) || start){
      if((ball_y > 0.0 && !is_ball_left) || is_ball_right){
        is_ball_right = true;
        slide_cmd = ball_y - 0.4;
      }
      if((ball_y < 0.0 && !is_ball_right) || is_ball_left){
        is_ball_left = true;
        slide_cmd = ball_y + 0.4;
      }
    ...
    ROS_INFO("##############################");
    ROS_INFO("pos: %d, side: %d, ball_left: %d, ball_right: %d", changepos, changeside, is_ball_left, is_ball_right);
    ROS_INFO("##############################");
    if(s <= L1 + L2){
      theta2 = -M_PIf64 + acos((s_square - pow(L1,2) - pow(L2, 2)) / (-2*L1*L2));
      //theta2 = acos((-s_square + pow(L1,2) + pow(L2, 2)) / (2*L1*L2));
      beta = atan2(inv_z1-inv_z0, inv_y1-inv_y0);
      psi = acos((s_square + pow(L1,2) - pow(L2,2)) / (2*s*L1));
      
      theta1 = beta + psi;
      theta3 = phi - theta1 - theta2;
      ROS_INFO("s^2: %4.3f", s_square);
      ROS_INFO("y0: %4.3f, z0: %4.3f, y1: %4.3f, z1: %4.3f", inv_y0, inv_z0, inv_y1, inv_z1);
      ROS_INFO("value: %4.3f %4.3f %4.3f %4.3f %4.3f", theta2*57.3, beta*57.3, psi*57.3, theta1*57.3, theta3*57.3);
      //ROS_INFO("value: %4.3f %4.3f %4.3f %4.3f %4.3f", theta2, beta, psi, theta1, theta3);
